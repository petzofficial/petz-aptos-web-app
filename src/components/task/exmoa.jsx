...
const EditAddTask = ({ method }) => {
  ...
  const manageEnergy = () => {
    const userData = JSON.parse(localStorage.getItem('userData')) || { energy: MAX_ENERGY, lastRechargeTime: new Date() };

    // Recharge energy every 5 minutes
    const currentTime = new Date();
    const timeSinceLastRecharge = currentTime - new Date(userData.lastRechargeTime);
    const energyRechargeCount = Math.floor(timeSinceLastRecharge / ENERGY_RECHARGE_INTERVAL);

    if (energyRechargeCount > 0) {
      userData.energy = Math.min(MAX_ENERGY, userData.energy + energyRechargeCount);
  ...
  const distributeReward = (task) => {
    const completedTasks = getTaskData().filter((item) => item.status === 'Completed');
        ...
        <div className='task-edit-inner mt-[-40px]'>
          ...
          <form
            ...
            <textarea
              ...
              defaultValue={task ? task.description : ''}></textarea>
