    ...
    return (
        <div className="setting-inner lg:mb-16 max-lg:mb-8">
            ...
            <div className="setting-left">
                ...
                <div className={`setting-left-navbar flex flex-col lg:space-y-10 space-y-7 ${outfit.className}`}>
                    {items.map((item) => {
                        return <div key={item.id} className="nav-item flex items-center">
                            <div className="s-icon text-[20px]">
                                <item.Icon />
                            </div>
                            <div className="flex items-center flex-1">
                                <Link href={`/setting/${item.slug}`}>
                                    <h4 className={`ml-3 cursor-pointer xl:!w-[200px] ${item.slug === method ? 'text-[#FF6900]' : ''}`}>{item.name}</h4>
                                </Link>
                                <IoIosArrowForward />
                            </div>
                        </div>
